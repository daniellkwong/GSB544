...
```{python}
data_2017 = data[(data['year'] == 2017) & (data['type'] == 'organic')]
...
The major geographical region that sold the most total organic, small Hass avacados in 2017 was **West**.
...
```{python}
summary_stats = (
    data_california.groupby(['geography', 'type'])['average_price']
    .agg(['mean', 'std', 'count'])
    .reset_index()
)
...
```{python}
price_diff = summary_stats.pivot(index='geography', columns='type', values='mean')
price_diff['diff'] = price_diff['organic'] - price_diff['conventional']

print(price_diff.sort_values('diff', ascending=False))
```

**San Francisco** has the largest difference in price between conventional and organic avocados. This is because the difference in mean price between the two types of avocados in San Francisco is the greatest, compared to the other California regions.  

```{python}
from plotnine import *

(
    ggplot(data_california, aes(x="geography", y="average_price", fill="type"))
    + geom_boxplot()
    + scale_y_continuous(breaks=[0.5, 1, 1.5, 2, 2.5, 3, 3.5, 4])  
    + labs(
...
```{python}
data_long = data_california.melt(
...
size_map = {"small_hass": "Small", "large_hass": "Large", "xl_hass": "Xlarge"}
data_long["size"] = data_long["size"].map(size_map)
```

```{python}
data_means = (
    data_long.groupby(["geography", "type", "size"])["volume"]
    .mean()
    .reset_index()
)

data_means["proportion"] = (
    data_means.groupby(["geography", "type"])["volume"]
    .transform(lambda x: x / x.sum()) # help from ChatGPT
)

data_proportion = data_means[["geography", "type", "size", "proportion"]]
```
...
(
    ggplot(data_proportion, aes(x="geography", y="proportion", fill="size"))
    ...
    + scale_y_continuous(
    labels=["0%", "25%", "50%", "75%", "100%"], # help from ChatGPT
...
```{python}
housing_data = pd.read_csv("/Users/danielkwong/Downloads/City_MedianRentalPrice_1Bedroom.csv")

housing_data = housing_data.drop(columns=["Unnamed: 0"], errors="ignore")

housing_long = housing_data.melt(
    id_vars=["RegionName", "State", "Metro", "CountyName", "SizeRank"],
    var_name="date",
...
california_housing = housing_long[housing_long["RegionName"].isin(ca_regions)]

data_california["date"] = pd.to_datetime(data_california["date"])

merged = pd.merge(
    data_california,
    california_housing,
    left_on=["geography", "date"],
...
(
    ggplot(merged, aes(x="rent_price", y="average_price", color="RegionName"))
    + geom_point(alpha=0.6)
    + labs(
        title="Avocado Prices vs Housing Rental Prices in California Metros",
        x="Median Rent Price (1-Bedroom Apartment)",
...
Avocado prices fluctuate a lot week-to-week, while rent prices rise steadily over time. This means there isn’t a strong direct relationship between how much people pay for housing and how much they pay for avocados, and that the two are independent of each other. Millennials can't afford houses simply because housing prices are rising, not because of avocado toast. 